...
export default async function SchedulePage({
  ...
  <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
    {events.map((event: GoogleCalendarEvent) => {
      ...
      return (
        <Card key={event.id} className="shadow-lg transition-transform transform hover:scale-105">
          <CardHeader className="bg-primary text-white rounded-t-lg p-4">
            <CardTitle className="text-xl font-semibold">{medico}</CardTitle>
          </CardHeader>
          <CardContent className="p-4 bg-white">
            <div className="grid grid-cols-1 gap-2">
              ...
              <p className="text-sm text-gray-600">
                Início: {new Date(event.start.dateTime).toLocaleString()} <br />
          ...
          <CardFooter className="flex justify-between p-4 bg-gray-100 rounded-b-lg">
            ...
            <Link href={`/attendance/${event.attendees[0].email}`}>
  ...
  <p className="text-center text-gray-500">Nenhum agendamento encontrado para esta data.</p>
)}
